...
namespace latency_benchmark
{
    ...
    public:
        ...
        [[nodiscard]] std::int64_t nanosecondsBetween(stamp_type start, stamp_type end) const noexcept;
...
#if LATENCY_BENCHMARK_HAS_TSC
    inline CycleClock::CycleClock()
        : nsPerCycle_(calibrateNsPerCycle())
    {}
    ...
    inline std::int64_t CycleClock::nanosecondsBetween(stamp_type start, stamp_type end) const noexcept
    {
    ...
    void readerLoop(Q&& q, int nIter, loopResult& results, std::latch& sync, const CycleClock& clock) {
        sync.wait();
    ...
    template <class Q>
    requires lockedin::detail::QueueInterface<Q, int>
    void writerLoop(Q&& q, int nIter, loopResult& results, std::latch& sync, const CycleClock& clock) {
        sync.wait();
        for (int i=0; i<nIter; i++) {
            auto start = clock.now();
...
int main()
{
    ...
    auto avgReader  = std::accumulate(rResults.begin(), rResults.end(), 0LL, [](auto a, const auto& b){return a+latency_benchmark::averageLatency(b);});
    auto succReader = std::accumulate(rResults.begin(), rResults.end(), 0LL, [](auto a, const auto& b){return a+latency_benchmark::nSuccesses(b);});
    auto avgWriter  = std::accumulate(wResults.begin(), wResults.end(), 0LL, [](auto a, const auto& b){return a+latency_benchmark::averageLatency(b);});
    auto succWriter = std::accumulate(wResults.begin(), wResults.end(), 0LL, [](auto a, const auto& b){return a+latency_benchmark::nSuccesses(b);});

    std::cout << "Reader latency average (ns): " << avgReader << '\n';
    std::cout << "Reader success rate:         " << succReader <<"/"<< iterations * readers << "(" << 100.0 * succReader / iterations / readers << "%)\n";
    std::cout << "Writer latency average (ns): " << avgWriter << '\n';
    std::cout << "Writer success rate:         " << succWriter <<"/"<< iterations * readers << "(" << 100.0 * succWriter / iterations / writers << "%)\n";
